    ...
    public class Evader
    {
        ...
        private void AIHeroClient_OnProcessSpellCast(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
        {
            if (evade != null && getCheckBoxItem("EVADEENABLE") && sender.Type == GameObjectType.AIHeroClient)
            {
                if (sender.IsEnemy)
                {
                    var sender_pos = sender.ServerPosition.LSTo2D();
        ...
        public void OnSpellHitDetected(Vector2 direction, AIHeroClient target)
        {
            ...
            var evade_pos = ObjectManager.Player.ServerPosition.LSTo2D() + evade_direction*EvadeSpell.Range;
        ...
        public void EvadeThread()
        {
            ...
            while (true)
            {
                try
                {
                    ...
                    {
                        var my_pos = ObjectManager.Player.ServerPosition.LSTo2D();
                            ...
                            else
                            {
                                    ...
                                    getCheckBoxItem("SHIELDENABLED"))
                                {
                                    ...
                                    if (allies != null)
                                    {
                                        ...
                                        {
                                            var ally_pos = ally.ServerPosition.LSTo2D();
        ...
        private bool CorrectNearTurret(ref Vector2 evade_pos, Vector2 direction)
        {
            ...
            if (turret != null)
            {
                if (turret.ServerPosition.LSTo2D().LSDistance(evade_pos) >
                    turret.ServerPosition.LSTo2D()
                        .LSDistance(ObjectManager.Player.ServerPosition.LSTo2D() - direction*EvadeSpell.Range))
                {
