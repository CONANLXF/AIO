    ...
    class WallJump
    {
        ...
        internal static void OnUpdate()
        {
            if (Jumping && jumpspot != null)
            {
                ...
                {
                    if (getItem != null && getItem.SpellSlot.IsReady() && jumpspot.CastPos.LSDistance(jumpspot.From) <= 400)
                    {
        ...
        internal static void OnWndProc(WndEventArgs args)
        {
            if (args.Msg == (uint)WindowsMessages.WM_LBUTTONDOWN && SpellSlot.E.IsReady())
            {
                ...
                if (jumpoint != null)
                {
                    Jumping = true;
                    jumpspot = jumpoint;
                    EloBuddy.Player.IssueOrder(GameObjectOrder.MoveTo, jumpoint.From);
                }
            }
