...
using TargetSelector = PortAIO.TSManager; namespace OAnnie
{
    internal class DrawManager : Annie
    {
        ...
        internal static void Drawing_OnDrawChamp(EventArgs args)
        {
            ...
            if (!GlobalManager.getCheckBoxItem(drawMenu, "FillDamage"))
                return;
            ...
            foreach (var unit in HeroManager.Enemies.Where(h => h.IsValid && h.IsHPBarRendered))
            {
                ...
                if (GlobalManager.getCheckBoxItem(drawMenu, "RushDrawWDamageFill"))
                {
        ...
        {
            var draw = GlobalManager.getCheckBoxItem(drawMenu, "Draw");
            var useq = GlobalManager.getCheckBoxItem(drawMenu, "qDraw");
            var usew = GlobalManager.getCheckBoxItem(drawMenu, "wDraw");
            var user = GlobalManager.getCheckBoxItem(drawMenu, "rDraw");
            var userf = GlobalManager.getCheckBoxItem(drawMenu, "rfDraw");
