 ...
 namespace jesuisFiora
{
    internal static class SpellBlock
    {
        ...
        {
            if (!Program.getKeyBindItem(Program.wSpells, "WSpells") || !SpellManager.W.IsReady())
            {
                return;
            }

            foreach (var skillshot in TreeLib.SpellData.Evade.GetSkillshotsAboutToHit(ObjectManager.Player, (int) (SpellManager.W.Delay * 1000f + Game.Ping / 2f)))
            {
                ...
                {
                    var item = getCheckBoxItem(Menu, enemy.ChampionName + spell.MenuName);
                    if (Menu[enemy.ChampionName + spell.MenuName] == null || !item)
                    {
                        continue;
                    }
        ...
        public static bool Contains(AIHeroClient unit, GameObjectProcessSpellCastEventArgs args)
        {
            ...
            if (args.SData.Name.Equals("KalistaRAllyDash") && getCheckBoxItem(Menu, "Oathsworn"))
            {
                return true;
            }
            ...
            foreach (var spell in
                spells)
            {
                var item = getCheckBoxItem(Menu, name + spell.MenuName);
                if (Menu[name + spell.MenuName] == null || !item)
                {
                    continue;
                }
