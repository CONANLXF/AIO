 ...
 namespace Marksman.Common
{
    public class CommonObjectDetector
    {
        ...
        private readonly List<ObjectDraw> objectDrawings = new List<ObjectDraw>();
        public CommonObjectDetector()
        {
        ...
        private void GameOnOnUpdate(EventArgs args)
        {
                ...
                ChampionSpell.IsReady())
            {
                foreach (
                    var ability in
                        objectDrawings.Where(d => d.Object.IsValid && d.Position.LSIsOnScreen() && d.End > Game.Time))
                {
        ...
        private void DrawingOnOnEndScene(EventArgs args)
        {
            ...
            foreach (var ability in objectDrawings.Where(d => d.Object.IsValid && d.Position.LSIsOnScreen() && d.End > Game.Time))
            {
                ...
                var buffTime = ability.End - Game.Time;

                float percent = (Math.Abs(ability.End - ability.Start) > float.Epsilon) ? buffTime / (ability.End - ability.Start) : 1f;
                //Game.PrintChat(percent.ToString());
        ...
        private void GameObjectOnOnDelete(GameObject sender, EventArgs args)
        {
            ...
            objectDrawings.RemoveAll(i => i.Object.NetworkId == sender.NetworkId || Game.Time > i.End);
        }
        ...
        private void GameObjectOnOnCreate(GameObject sender, EventArgs args)
        {
            ...
            if (objects.TryGetValue(sender.Name.ToLower(), out ability))
            {
                //                if (ability.Enabled)
                //              {
                objectDrawings.Add(new ObjectDraw { Object = sender, Start = Game.Time, End = Game.Time + ability.Time, Color = ability.Color });
                //            }
            }
        }
    }
