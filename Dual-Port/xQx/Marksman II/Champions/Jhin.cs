...
using TargetSelector = PortAIO.TSManager; namespace Marksman.Champions
{
    internal class Jhin : Champion
    {
        ...
        private static bool JhinQCastReady
        {
            get
            {
                return ObjectManager.Player.HasBuff("JhinQCastReady");
            }
        }
        ...
        public override void Game_OnGameUpdate(EventArgs args)
        {
            ...
            if (HarassActive)
            {
                ...
                if (Program.harass["UseWH"].Cast<CheckBox>().CurrentValue && W.IsReady()) W.Cast(target);
            }
                ...
                if (jQ != 0)
                {
                    var totalAa =
                        ObjectManager.Get<Obj_AI_Minion>()
                            .Where(
                                m => m.IsEnemy && !m.IsDead && m.LSIsValidTarget(Orbwalking.GetRealAutoAttackRange(null)))
        ...
        {
            config.Add("Draw.X", new Slider("W range", 500, 0, 4000));
            return true;
        }

        private static void DrawingOnOnEndScene(EventArgs args)
        {
            LeagueSharp.Common.Utility.DrawCircle(ObjectManager.Player.Position, W.Range, System.Drawing.Color.White, 1, 23, true);
            LeagueSharp.Common.Utility.DrawCircle(ObjectManager.Player.Position, R.Range, System.Drawing.Color.Aqua, 1, 23, true);
            var rCircle2 = Program.marksmanDrawings["Draw.X"].Cast<Slider>().CurrentValue;
            LeagueSharp.Common.Utility.DrawCircle(ObjectManager.Player.Position, rCircle2, System.Drawing.Color.Red, 1, 23, true);
        }

        public override bool MiscMenu(Menu config)
        {
            config.Add("RInterruptable", new CheckBox("Auto R Interruptable Spells"));
            config.Add("EFlash", new CheckBox("Use E against FlJhins"));
        ...
        {
            var drawx = Program.marksmanDrawings["Draw.X"].Cast<Slider>().CurrentValue;
